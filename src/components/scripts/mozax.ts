...
//     WebGLRenderer,
//     sRGBEncoding,
//     Group, PerspectiveCamera, DirectionalLight, PointLight
// } from "three";
...
// /** containers to hold the threejs experiences **/
...
// loader.load("/models/piano.glb", (gltf) => {
...
//     fillLight.position.y -= (parallaxY * 9 + fillLight.position.y - 1) * deltaTime;
...
loader.load("models/piano.glb", function (gltf) {
    gltf.scene.traverse((obj) => {
        if (obj.isMesh) {
            oldMaterial = obj.material;
            obj.rotation.z = Math.PI / 2;
